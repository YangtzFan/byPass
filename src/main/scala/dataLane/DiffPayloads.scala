...
class RenameEntry extends DecodedInst { // 其他变量直接继承自上一阶段
  ...
  // ready 初值由 IQ 入队当拍读取 ReadyTable 获得；Rename 不再透传 psrcReady。
}
...
class Rename_Dispatch_Payload extends Bundle {
  val entries = Vec(CPUConfig.renameWidth, new RenameEntry)
...
// ==========================================================================
// Issue_ReadReg_Lane：Issue → ReadReg 单 lane 数据内容
// ==========================================================================
// Issue 阶段从 IssueQueue 取出 1 条指令，传给 ReadReg 阶段读取物理寄存器。
// 阶段 2 起：本 Bundle 仅代表“单 lane 的内容”，外层用 Vec(issueWidth, _) 统一包裹。
// ==========================================================================
class Issue_ReadReg_Lane extends Bundle {
  val pc             = UInt(32.W) // 指令 PC
  val inst           = UInt(32.W) // 32 位指令
  val imm                  = UInt(32.W) // 符号扩展后的立即数
  val type_decode_together = UInt(9.W)  // 指令类型独热编码 {U,JAL,JALR,B,L,I,S,R,Other}
...
// ==========================================================================
// Issue_ReadReg_Payload：Issue → ReadReg 多 lane 流水线数据包
// ==========================================================================
// 阶段 2 统一接口：`lanes` 为每 lane 的内容，`validMask(i)` 表示第 i 个 lane
// 的 payload 是否有效。外层 Decoupled.valid 表示整包是否在流动，两者共同决定
// 下游是否处理某条 lane 的数据。issueWidth=1 时 validMask 宽度为 1，与 scalar
// 语义等价。
// ==========================================================================
class Issue_ReadReg_Payload extends Bundle {
  val lanes     = Vec(CPUConfig.issueWidth, new Issue_ReadReg_Lane)
  val validMask = UInt(CPUConfig.issueWidth.W)
}

// ==========================================================================
// ReadReg_Execute_Lane / Payload：ReadReg → Execute 流水线数据
// ==========================================================================
// ReadReg 阶段读取寄存器堆获得 rs1/rs2 的值，作为旁路链的兜底值
// ==========================================================================
class ReadReg_Execute_Lane extends Issue_ReadReg_Lane {
...
class ReadReg_Execute_Payload extends Bundle {
  val lanes     = Vec(CPUConfig.issueWidth, new ReadReg_Execute_Lane)
  val validMask = UInt(CPUConfig.issueWidth.W)
}

// ==========================================================================
// Execute_Memory_Lane / Payload：Execute → Memory 执行结果 + 分支验证信息
// ==========================================================================
class Execute_Memory_Lane extends Bundle {
  ...
  val actual_target        = UInt(32.W) // 实际跳转目标（Memory 用于 redirect.addr）
  val mispredict           = Bool()     // 是否预测错误（Memory 用于 redirect.valid）
...
class Execute_Memory_Payload extends Bundle {
  val lanes     = Vec(CPUConfig.memoryWidth, new Execute_Memory_Lane)
  val validMask = UInt(CPUConfig.memoryWidth.W)
}

// ==========================================================================
// Memory_Refresh_Lane / Payload：Memory → Refresh 访存结果
// ==========================================================================
class Memory_Refresh_Lane extends Bundle {
...
class Memory_Refresh_Payload extends Bundle {
  val lanes     = Vec(CPUConfig.refreshWidth, new Memory_Refresh_Lane)
  val validMask = UInt(CPUConfig.refreshWidth.W)
}
...
class SBAllocIO extends Bundle {
  ...
  val idxs         = Input(Vec(CPUConfig.renameWidth, UInt(CPUConfig.sbIdxWidth.W)))    // 分配的物理槽位索引（FreeList 返回）
  val storeSeqs    = Input(Vec(CPUConfig.renameWidth, UInt(CPUConfig.storeSeqWidth.W))) // 分配的 storeSeq 逻辑年龄
...
class ROBAllocData extends Bundle {
  ...
  val isSysStop     = Bool()     // 是否为 SYSTEM/FENCE/Other 指令（ECALL/EBREAK/FENCE 等：本拍提交后必须截断）
...
class ROBMultiAllocIO extends Bundle { // 方向以 Dispatch（发送方）为基准
  ...
  val data       = Output(Vec(CPUConfig.renameWidth, new ROBAllocData))             // 每条指令的分配数据
  val idxs       = Input(Vec(CPUConfig.renameWidth, UInt(CPUConfig.robPtrWidth.W))) // ROB 返回的分配指针
}
...
// ==========================================================================
// IQ 唤醒接口（Refresh / 其它写回源 → IssueQueue 广播）
// ==========================================================================
// 每个唤醒源携带：
//   valid : 本拍是否产生一个有效写回
//   pdst  : 该写回目标的物理寄存器编号（与 IQ 中 psrc1/psrc2 比较）
//
// IQ 在下一拍看到匹配的条目时，将 src{1,2}Ready 置 1。
// 当前后端仍保持 1 宽，只需单条 wakeup 源（来自 Refresh），预留参数化以便
// 后续扩展到多写回源（Memory 旁路 / Commit 旁路 / 多 lane Refresh 等）。
class IQWakeupSource extends Bundle {
  val valid = Bool()
