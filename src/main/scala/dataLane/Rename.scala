...
class Rename extends Module {
  ...
  // ---- 逐路处理：生成 RenameEntry ----
  for (i <- 0 until 4) {
    val td    = in.bits.insts(i).type_decode_together
    ...
    val iType = td(3)
    val rType = td(1)
    // 计算是否需要写回寄存器（U 型、JAL、JALR、Load、I 型、R 型均需写回）
    val regWriteEnable = uType || jal || jalr || lType || iType || rType

    // ---- 向下游输出 RenameEntry ----
    out.bits.entries(i).pc                   := in.bits.insts(i).pc
    out.bits.entries(i).inst                 := in.bits.insts(i).inst
    out.bits.entries(i).imm                  := in.bits.insts(i).imm
    out.bits.entries(i).type_decode_together := in.bits.insts(i).type_decode_together
    out.bits.entries(i).predict_taken        := in.bits.insts(i).predict_taken
    out.bits.entries(i).predict_target       := in.bits.insts(i).predict_target
    out.bits.entries(i).bht_meta             := in.bits.insts(i).bht_meta
