...
//
// 指令内容不再在 Chisel 编译期从特定测试用例加载。编译时仅使用占位 hex 文件
// 防止 FIRRTL 将内存优化掉。实际测试用例由 difftest 框架在 RTL 后处理阶段
// 替换 $readmemh 路径，实现运行时动态加载，避免切换测试用例时重新编译 RTL。
// ============================================================================
...
// ============================================================================
// 在 Chisel 编译期创建一个最小的占位 hex 文件（单行全零），
// 仅用于防止 FIRRTL 优化掉 IROM 内存。
// 实际测试用例的加载由 difftest 框架负责。
// ============================================================================
object IROM {
