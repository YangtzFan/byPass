...
int check(WB_info stu, WB_info ref) {
    ...
    if(fail) {
        ...
        printf("[SAME] Test Passed in this step!\n");
        printf("============== SAME =============\n");
        printf("SIGNAL NAME\tREFERENCE\tMYCPU\n");
        printf("debug_wb_pc\t0x%8.8x\t0x%8.8x\n", ref.wb_pc, stu.wb_pc);
        printf("debug_wb_ena\t%10d\t%10d\n", ref.wb_ena, stu.wb_ena);
...
int main(int argc, char** argv, char** env) {
    ...
    for(int i = 0; i < 10000; i++) {
        ...
        if(rtl_wb_info.wb_have_inst) {
            if(check(rtl_wb_info, cpu_run_once()) == -1) break;
        } else {
            printf("[SKIP] Skipped this step!\n");
            printf("============== SKIP =============\n");
            printf("The wb_have_inst in current timing zone is %d\n", rtl_wb_info.wb_have_inst);
        }
    }
